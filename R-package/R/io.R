...
BucketIter <-setRefClass("BucketIter",
                         fields = c("buckets", "bucket.names", "batch.size",
                                    "data.mask.element", "shuffle", "bucket.plan",
                                    "bucketID", "epoch", "batch", "batch.per.epoch",
                                    "seed"),
                         methods = list(
                           initialize = function(buckets, batch.size,
                                                 data.mask.element = 0,
                                                 shuffle = FALSE,
                                                 seed = 123) {
                           ...
                           reset = function() {
                             ...
                             if (.self$shuffle) {
                               set.seed(.self$seed)
                               bucket_plan_names <- sample(rep(names(batch_per_bucket),
                                                               times = batch_per_bucket))
                               .self$bucket.plan <- ave(bucket_plan_names == bucket_plan_names,
                                                        bucket_plan_names, FUN = cumsum)
                               ...
                               bucket_plan_names <- rep(names(batch_per_bucket), 
                                                        times = batch_per_bucket)
                               .self$bucket.plan <- ave(bucket_plan_names == bucket_plan_names,
                                                        bucket_plan_names, FUN = cumsum)
                               names(.self$bucket.plan) <- bucket_plan_names
                           ...
                           value = function() {
                             ...
                             data_mask <- as.integer(names(.self$bucketID)) - apply(data == .self$data.mask.element, 2, sum)
...
mx.io.bucket.iter <- function(buckets, batch.size, data.mask.element = 0,
  ...
  return(BucketIter$new(buckets = buckets,
